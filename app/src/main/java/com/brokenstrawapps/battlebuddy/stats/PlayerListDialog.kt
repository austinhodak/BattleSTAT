...
import com.brokenstrawapps.battlebuddy.stats.compare.ComparePlayerModel
import com.brokenstrawapps.battlebuddy.stats.main.StatsHome
import com.brokenstrawapps.battlebuddy.stats.scanner.ScannedResults
...
class PlayerListDialog : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mAdapter = SlimAdapter.create().attachTo(playerListRV).updateData(players).register<PlayerListModel>(R.layout.player_list_item_updated) { player, injector ->
            val cardView = injector.findViewById<CardView>(R.id.playerListCard)

            val rankRibbon = injector.findViewById<ImageView>(R.id.rankRibbon)
            val rankMedal = injector.findViewById<ImageView>(R.id.rankIcon)
            ...
            if (mSharedPreferences?.contains("playerRankTitle-${player.playerID}") == true) {
                ...
                val rankString = mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "0-0")!!

                Glide.with(applicationContext)
                        .load(Ranks.getRankRibbon(rankString))
                ...
                Glide.with(applicationContext)
                        .load(Ranks.getRankMedal(rankString))
                        //.load(Ranks.getRankIcon(Rank.valueOf(mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "UNKNOWN")!!.toUpperCase())))
                        .transition(DrawableTransitionOptions.withCrossFade(factory))
                        .into(rankMedal)
                ...
                injector.invisible(R.id.rankIcon)
                injector.invisible(R.id.rankRibbon)
            ...
            listeners[FirebaseDatabase.getInstance().getReference("user_stats/${player.playerID}/season_data/${player.getDatabaseSearchURL()}/${Seasons.getCurrentSeasonForPlatform(player.platform).codeString}/stats")] = FirebaseDatabase.getInstance().getReference("user_stats/${player.playerID}/season_data/${player.getDatabaseSearchURL()}/${Seasons.getCurrentSeasonForPlatform(player.platform).codeString}/stats").addValueEventListener(object : ValueEventListener {
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    if (!p0.exists()) {
                        if (!isDestroyed) {
                            Glide.with(applicationContext)
                                    .load(Ranks.getRankRibbon(null))
                                    //.load(Ranks.getRankIcon(Rank.valueOf(mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "UNKNOWN")!!.toUpperCase())))
                                    .transition(DrawableTransitionOptions.withCrossFade(factory))
                                    .into(rankRibbon)

                            Glide.with(applicationContext)
                                    .load(Ranks.getRankMedal(null))
                                    //.load(Ranks.getRankIcon(Rank.valueOf(mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "UNKNOWN")!!.toUpperCase())))
                                    .transition(DrawableTransitionOptions.withCrossFade(factory))
                                    .into(rankMedal)
                        }
                    ...
                    injector.visible(R.id.rankRibbon)
                    injector.visible(R.id.rankIcon)

                    Glide.with(applicationContext)
                            .load(Ranks.getRankRibbon(seasonStats.getHighestRankTitle()))
                            //.load(Ranks.getRankIcon(Rank.valueOf(mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "UNKNOWN")!!.toUpperCase())))
                            .transition(DrawableTransitionOptions.withCrossFade(factory))
                            .into(rankRibbon)

                    Glide.with(applicationContext)
                            .load(Ranks.getRankMedal(seasonStats.getHighestRankTitle()))
                            //.load(Ranks.getRankIcon(Rank.valueOf(mSharedPreferences?.getString("playerRankTitle-${player.playerID}", "UNKNOWN")!!.toUpperCase())))
                            .transition(DrawableTransitionOptions.withCrossFade(factory))
                            .into(rankMedal)
        ...
        playerListFAB?.setOnActionSelectedListener {
            ...
            if (Premium.isUserLevel2() && players.size >= 25) {
            ...
            } else if (Premium.isUserLevel1() && players.size >= 15) {
                Snacky.builder().setActivity(this).warning().setText("Player Limit Reached").setAction("UPGRADE") {
                    startActivity<UpgradeActivity>()
                }.show()
                return@setOnActionSelectedListener true
            } else if (!Premium.isPremiumUser() && players.size >= 10) {
                Snacky.builder().setActivity(this).warning().setText("Player Limit Reached").setAction("UPGRADE") {
                    startActivity<UpgradeActivity>()
                }.show()
                return@setOnActionSelectedListener true
        ...
        when (Premium.getUserLevel()) {
            Premium.Level.FREE -> playerListLimitTV.text = "LIMIT OF 10 PLAYERS  \uD83C\uDF57"
            Premium.Level.LEVEL_1 -> playerListLimitTV.text = "LIMIT OF 15 PLAYERS  \uD83C\uDF57"
            Premium.Level.LEVEL_2 -> playerListLimitTV.text = "LIMIT OF 25 PLAYERS  \uD83C\uDF57"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1 && resultCode == Activity.RESULT_OK) {
            startActivity<ScannedResults>("path" to currentPhotoPath)
        }
    }
