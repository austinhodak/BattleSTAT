...
import com.ahcjapps.battlebuddy.weapondetail.WeaponDetailTimeline
    ...
    private fun setupAdapter(int: Int) {
        ...
        mAdapter = SlimAdapter.create().attachTo(weapon_list_rv).register(R.layout.weapon_item) { doc: DataSnapshot, injector ->
            ...
            if (data.icon.isNotEmpty() && data.icon != "--") {
                val gsReference = FirebaseStorage.getInstance().getReferenceFromUrl(data.icon.replace("pubg-center", "battlegrounds-battle-buddy"))
